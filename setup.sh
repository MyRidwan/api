...
echo $uuid > /root/api/key.txt
mkdir /var/lib/crot;
mkdir /etc/xray
read -rp "INPUT YOUR DOMAIN : " domain
printf %s $domain > domain
printf %s $domain > /etc/xray/domain
echo {} > /etc/xray/user.json
echo "IP=" >> /var/lib/crot/ipvps.conf
#install tools/alat
chmod +x install-tools.sh && ./install-tools.sh
#
#Instal Xray
chmod +x install-xray.sh && ./install-xray.sh
#install xmenu
mv menu /usr/bin/menu
chmod +x /usr/bin/menu
mv certv2ray /usr/bin/certv2ray
chmod +x /usr/bin/certv2ray
mv restart-xray /usr/bin/restart-xray
chmod +x /usr/bin/restart-xray
mv change-domain /usr/bin/change-domain
chmod +x /usr/bin/change-domain
certv2ray
#SELESAI
echo " "
...
echo -e "    ARH-PROJECT TEAM     "  | tee -a log-install.txt
echo -e "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
